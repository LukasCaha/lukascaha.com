...
const useStyles = (c: string, blur: number = 20) =>
  makeStyles((theme: Theme) => ({
    glass: {
      backgroundColor: color(c).alpha(0.1).toString(),
      backgroundImage: `linear-gradient(to bottom right, ${color(c)
        .alpha(0.2)
        .toString()}, ${color(c).alpha(0).toString()}), url(grain.svg)`,
      backdropFilter: `blur(${blur}px)`,
      boxShadow: "10px 10px 10px rgba(30, 30, 30, 0.1)",
    },
    glassBorders: {
      borderLeft: `solid 2px ${color(c).alpha(0.3).toString()}`,
